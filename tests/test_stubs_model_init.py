...
def stubs_model(stubs_mesh):
    ...
    rc.add([
        ...
        Reaction('B -> 0',     ['B'], [],
                 {'on': 'kdeg_B'}, reaction_type='mass_action_forward'),
        Reaction('X_pm -> 0',  ['X_pm'], [],
                 {'on': 'kdeg_Xpm'}, reaction_type='mass_action_forward'),
        Reaction('X_erm -> 0', ['X_erm'], [],
                 {'on': 'kdeg_Xerm'}, reaction_type='mass_action_forward'),
    ])
...
def test_stubs_model_init(stubs_model):
    ...
    for idx in test_indices:  # few different indices
        ...
        assert (a == b).all()
        assert (pm_mesh.cell_coordinates[idx] ==
                parent_mesh.facet_coordinates[pidx]).all()
