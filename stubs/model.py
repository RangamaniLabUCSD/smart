    ...
    def _init_4_7_set_initial_conditions(self):
        ...
        for species in self.sc:
            for ukey in species.u.keys():
                if isinstance(species.initial_condition, float) or not isinstance(species.initial_condition, str):
        ...
        if self.config.solver["use_snes"]:
            ...
            if self.config.solver["reset_timestep_for_negative_solution"]:
                ...
                if negVals:
                    ...
                    # need to re-link global function with species-specific functions after re-setting previous solution
                    self._init_4_4_get_species_u_v_V_dofmaps()
                    self._init_4_5_name_functions()
    ...
    def dolfin_set_function_values(self, sp, ukey, unew):
        ...
        if isinstance(unew, d.Expression):
            ...
            if len(sp.dof_map)==len(unew):
                ...
                #nearest neighbor interpolation (because currently evaluated on same mesh, just scrambled order)
                ...
                for i in range(len(dof_coord)):
                    ...
                    if i%1000==0:
                        fancy_print(f"Set {i} of {len(dof_coord)} function values for {sp.name}")
                #indices = self.dolfin_get_dof_indices(sp)
                indices = sp.dof_map
                uvec    = u.vector()
