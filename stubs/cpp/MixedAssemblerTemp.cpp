...
//----------------------------------------------------------------------------
void MixedAssemblerTemp::assemble(GenericTensor& A, const Form& a)
{
...
//-----------------------------------------------------------------------------
void MixedAssemblerTemp::assemble_cells(
  GenericTensor& A,
  const Form& a,
  UFC& ufc,
  ...
  std::vector<double>* values)
{
  ...
  // Set timer
  Timer timer("[MixedAssemblerTemp] Assemble cells");
  ...
  for (CellIterator cell(mesh); !cell.end(); ++cell)
  {
	...
	else if(codim[i] == 1) // 2D-1D or 3D-2D (cells - facets relationships)
	{
	  ...
	  std::cout << "[MixedAssemblerTemp] codim 2 - Not implemented (yet)" << std::endl;
      }
    }
...
//-----------------------------------------------------------------------------
void MixedAssemblerTemp::assemble_exterior_facets(
  GenericTensor& A,
  const Form& a,
  UFC& ufc,
  std::shared_ptr<const MeshFunction<std::size_t>> domains,
...
//-----------------------------------------------------------------------------
void MixedAssemblerTemp::assemble_interior_facets(
  GenericTensor& A,
  const Form& a,
  UFC& ufc,
  std::shared_ptr<const MeshFunction<std::size_t>> domains,
...
//-----------------------------------------------------------------------------
void MixedAssemblerTemp::assemble_vertices(
  GenericTensor& A,
  const Form& a,
  UFC& ufc,
  std::shared_ptr<const MeshFunction<std::size_t>> domains)
{
  ...
  for (std::size_t i = 0; i < form_rank; ++i)
  {
    ...
    {
      dolfin_error("MixedAssemblerTemp.cpp",
...
{
  // dolfin::MixedAssemblerTemp
  py::class_<dolfin::MixedAssemblerTemp, std::shared_ptr<dolfin::MixedAssemblerTemp>, dolfin::AssemblerBase>
    (m, "MixedAssemblerTemp", "DOLFIN MixedAssemblerTemp object")
    .def(py::init<>())
    .def("assemble", &dolfin::MixedAssemblerTemp::assemble)
    .def("assemble_cells", &dolfin::MixedAssemblerTemp::assemble_cells)
    .def("assemble_interior_facets", &dolfin::MixedAssemblerTemp::assemble_interior_facets)
    .def("assemble_exterior_facets", &dolfin::MixedAssemblerTemp::assemble_exterior_facets)
    .def("assemble_vertices", &dolfin::MixedAssemblerTemp::assemble_vertices);

  // py::class_<MixedAssemblerTemp> (m, "PETScDMCollectionTemp")
  //   // Initialization
