...
def sbml_unit_to_pint_quantity(sbml_unit):
    kind = sbml_unit.kind
...
def sbml_unit_definition_to_pint_quantity(sbml_unit_definition):
    quantity = 1 * unit_registry.dimensionless
    for unit in sbml_unit_definition.getListOfUnits():
        quantity *= sbml_unit_to_pint_quantity(unit)
    return quantity.to_compact()

def pint_to_sbml_unit_definition(pint_unit, model):
...
# TODO: needs fixing to account for quantity return instead of unit return 
...
#         sid = unit_def.id
#         pint_unit = sbml_unit_definition_to_pint_quantity(unit_def)
...
pint_to_sbml_unit_definition(u_a.units, model)
pint_to_sbml_unit_definition(u_b.units, model)
pint_to_sbml_unit_definition(u_c.units, model)
