...
def mixed_problem():
    ...
    ds0 = d.Measure('ds', domain=mesh0); ds1 = d.Measure('ds', domain=mesh1);
    # initial condition
    ...
    F0 = u0*v0*dx0 + d.inner(d.grad(u0), d.grad(v0))*dx0 - u0n*v0*dx0 - (d.Constant(0.01)*u0*v0*dx_)
    F1 = u1*v1*dx1 + d.inner(d.grad(u1), d.grad(v1))*dx1 - u1n*v1*dx1 - (d.Constant(0.01)*v1*dx_)
...
def print_mpi_mesh_info(mesh):
    ...
    print(f"cpu {rank}: number of vertices/shared_vertices = {mesh.num_vertices()}, {len(shared_vertices)}")
    print(f"cpu {rank}: number of cells/shared_cells = {mesh.num_cells()}, {len(shared_cells)}")
...
for f in d.facets(mesh):
    mf2[f] = 11*(midpoint-d.DOLFIN_EPS <= f.midpoint().z() <= midpoint+d.DOLFIN_EPS)

mesh0 = d.MeshView.create(mf3, 1); mesh_ = d.MeshView.create(mf2, 11)
# build mesh mapping
