...
for dt in [0.012, 0.01, 0.008, 0.006, 0.004, 0.002]:
    ...
    cyto_mesh = stubs.mesh.Mesh(
        mesh_filename='/Users/rabona/Documents/stubs/examples/unit_cube.xml', name='cyto')
    ...
    # solve system
    model.solve()
    errors.append(np.max(model.u['cyto']['u'].vector().get_local(
    )-(lambda t: 10*np.exp(-5*t))(model.t))/(lambda t: 10*np.exp(-5*t))(model.t))
