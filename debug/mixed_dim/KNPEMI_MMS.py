...
class MMS:
    ...
    def get_MMS_terms_EMI(self, time):
        ...
        for key in exact_solutions:
            ...
            exec(f'global {key}; {key} = exact_solutions["{key}"]')

        # --------------------- Calculate components ------------------------ #
        # gradients
        grad_phii, grad_phie = (
            np.array([sm.diff(foo, x), sm.diff(foo, y)]) for foo in (phi_i_e, phi_e_e)
        )
        ...
        J_e = Expression(
            (sm.printing.ccode(grad_phie[0]), sm.printing.ccode(grad_phie[1])),
            t=time,
            degree=4,
        )
    ...
    def get_MMS_terms_KNPEMI(self, time):
        ...
        # equations for potentials: fE = - F sum(z_k*div(J_k_r)
        f_phi_i = -(
            (sm.diff(J_Na_i[0], x) + sm.diff(J_Na_i[1], y))
            + (sm.diff(J_K_i[0], x) + sm.diff(J_K_i[1], y))
            - (sm.diff(J_Cl_i[0], x) + sm.diff(J_Cl_i[1], y))
        )
        f_phi_e = -(
            (sm.diff(J_Na_e[0], x) + sm.diff(J_Na_e[1], y))
        ...
        J_Nae, J_Ke, J_Cle = (
            Expression(
                (sm.printing.ccode(foo[0]), sm.printing.ccode(foo[1])),
                t=time,
                degree=4,
            )
