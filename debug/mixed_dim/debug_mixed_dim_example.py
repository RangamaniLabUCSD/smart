...
)  # Integration on the blood vessel - velocity
dL_in = Measure(
    "dx",
    domain=W.sub_space(2).mesh(),
)  # Integration on the inlet boundary - LM_in
dL_out = Measure(
    "dx",
    domain=W.sub_space(3).mesh(),
)  # Integration on the outlet boundary - LM_out

# Boundaries as exterior facets from 2D mesh
dV_boundary = Measure(
    "ds",
    domain=W.sub_space(0).mesh(),
    subdomain_data=physical_facets,
)
# Boundaries for LMs
...
boundaries_LM_in = MeshFunction(
    "size_t",
    inlet_mesh,
    inlet_mesh.topology().dim() - 1,
    0,
)
DirichletBoundary().mark(boundaries_LM_in, 1)
# Mark boundary of LM_out
boundaries_LM_out = MeshFunction(
    "size_t",
    outlet_mesh,
    outlet_mesh.topology().dim() - 1,
    0,
)
...
L = (
    rho * inner(f, v) * dV
    - p0_in * inner(v, n) * dV_boundary(101)
    - p0_out * inner(v, n) * dV_boundary(102)
)
