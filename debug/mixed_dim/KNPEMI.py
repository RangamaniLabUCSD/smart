...
def setup_square_domain(n):
    r"""Inner (interior) is [0.25, 0.75]^2, outer (exterior) is
    ...
    for f in facets(mesh):
        interface_left = (
            0.25 - DOLFIN_EPS < f.midpoint().x() < 0.25 + DOLFIN_EPS
        ) and (0.25 - DOLFIN_EPS < f.midpoint().y() < 0.75 + DOLFIN_EPS)
        interface_right = (
            0.75 - DOLFIN_EPS < f.midpoint().x() < 0.75 + DOLFIN_EPS
        ) and (0.25 - DOLFIN_EPS < f.midpoint().y() < 0.75 + DOLFIN_EPS)
        interface_bottom = (
            0.25 - DOLFIN_EPS < f.midpoint().y() < 0.25 + DOLFIN_EPS
        ) and (0.25 - DOLFIN_EPS < f.midpoint().x() < 0.75 + DOLFIN_EPS)
        interface_top = (0.75 - DOLFIN_EPS < f.midpoint().y() < 0.75 + DOLFIN_EPS) and (
            0.25 - DOLFIN_EPS < f.midpoint().x() < 0.75 + DOLFIN_EPS
        )
        surfaces[f] = (
            interface_left or interface_right or interface_bottom or interface_top
        )
    gamma_mesh = MeshView.create(surfaces, 1)

    # Mark the outside of exterior mesh
    facet_f = MeshFunction(
        "size_t",
        exterior_mesh,
        exterior_mesh.topology().dim() - 1,
        0,
    )
    CompiledSubDomain("near(x[0]*(1-x[0]), 0) || near(x[1]*(1-x[1]), 0)").mark(
        facet_f,
        1,
    )
...
def solve_system(n, t, dt, Tstop, ion_list, M, order):
    ...
    for idx, ion in enumerate(ion_list):
        ...
        # ion fluxes
        Ji = -Constant(Di) * grad(ki) - Constant(Di * z / psi) * ki_prev * grad(
            phi_i,
        )  # linearised
        Je = -Constant(De) * grad(ke) - Constant(De * z / psi) * ke_prev * grad(
            phi_e,
        )  # linearised
        ...
        L1 += (
            F * z * inner(dot(ion["J_k_e"], n_outer), vphi_e) * dsOuter
        )  # eq for phi_e
    ...
    L2 += inner(phi_M_prev, q_IM) * dxGamma - dt / C_M * inner(I_ch, q_IM) * dxGamma
    ...
    L2 += (
        dt
        / C_M
        * sum(inner(JM, q_IM) * dxGamma(i) for i, JM in enumerate(ion["f_J_M"], 1))
    )
    ...
    for k in range(int(round(Tstop / float(dt)))):
        ...
        phi_M_prev.assign(
            interpolate(ui_p.sub(N_ions), Wg) - interpolate(ue_p.sub(N_ions), Wg),
        )
